...
def remove_ai_service():
    this_dir = os.path.dirname(os.path.abspath(__file__))
    target_ai_remove_file = os.path.join(
        this_dir, "AI/service/unInstall_SusiService.sh")
...
def install_library():
    ...
    for module in modules:
        os.system(
            f'cp -af "{this_dir}/modules/{module}" "{linux_susiiot_module_dir}/"')

    cpu_name = os.popen("cat /proc/cpuinfo | grep 'model name' | uniq").read()
    if "Intel" in cpu_name:
        os.system(
            f'cp -af "{this_dir}/modules/libSUSIAIIoT.so" "{linux_susiiot_module_dir}/"')
