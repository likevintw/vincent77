...
enum evtypeRank{
	...
	evtypePWCYCLE,	/* = SUSI_WDT_EVENT_TYPE_PWRCYCLE */
...
static void support_evtype_init(uint8_t iDevice)
{
	...
	if (info[iDevice].eventFlag)
	{
		...
		if (info[iDevice].eventFlag & SUSI_WDT_FLAG_SUPPORT_PWRCYCLE)
			evtype[index++] = evtypePWCYCLE;
...
static void wdog_status(uint8_t iDevice)
{
	uint32_t wdog_status = 0;
	SusiStatus_t status;
	SusiId_t id = devids[iDevice];

	status = SusiWDogGetCaps(id, SUSI_ID_WDT_EVENT_STATUS, &wdog_status);
	if (status == SUSI_STATUS_SUCCESS)
		printf("Watchdog is %s\n", (wdog_status == 1) ? "running." : "stopped.");
}
...
static uint8_t wdog_start(uint8_t iDevice)
{
	...
	if (status != SUSI_STATUS_SUCCESS)
	{
		if (status == SUSI_STATUS_RUNNING)
			printf("Watchdog started. (0x%08X)\n", status);
...
static void menu_start(int8_t *manuItem, uint8_t numManuItem, struct WDogInfo info)
{
	...
	if (info.maxEvent > 0 && info.eventFlag)
	{
		...
		for (j = 0; j < NumEventType && evtype[j] > SUSIDEMO_ITEM_UNDEFINED; j++)
		{
			...
			if (evtype[j] == info.eventType)
				printf("[%s]", evtypeStr[evtype[j]]);
			else
				printf(" %s ", evtypeStr[evtype[j]]);
		}
		printf("\n");
