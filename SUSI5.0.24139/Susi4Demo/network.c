...
uint8_t network_apply(void)
{
	SusiStatus_t status;
	uint32_t STORE_TO_FLASH = 0x10;

	status = SusiBoardSetValue(SUSI_ID_NETWORK_CONTROL, &STORE_TO_FLASH);
	if (status == SUSI_STATUS_SUCCESS)
		printf("Set STORE_TO_FLASH 0x%X success\n", STORE_TO_FLASH);
	else
		printf("Set STORE_TO_FLASH 0x%X failed(0x%X)\n", STORE_TO_FLASH, status);
...
uint8_t network_available(void)
{
	SusiStatus_t status;
	uint32_t getVal = 0;
...
uint8_t network_control(void)
{
	...
	if (input_uint(&op, 10, 2, 0))
	{
		printf("Invalid input\n");
		goto exit;
	}

	if (networkCtrlStr[op].contorl == 0x00 && network_available() == 0)
	{
		printf("Set 0x%X not support\n", networkCtrlStr[op].contorl);
		goto exit;
	}
...
void network_main(void)
{
	...
	while (1)
	{
		...
		if (functions[op] != SUSIDEMO_FUNCTIONS_GOBACK &&
			functions[op] != funcNetwrokControl &&
			functions[op] != funcNetwrokStatus && 
			network_available() == 0)
		{
			printf("Not support set %s\n", funcStr[functions[op]]);
			goto pause;
		}
		switch (functions[op])
		{
		...
		case funcNetworkMacAddr:
			printf("Not support set MAC address\n");
			//network_macaddr();
			goto pause;
